...
class ShiftScheduler:
    ...
    def __init__(self):
        ...
        self.objective = None  # 最適化結果の目的関数の値
        self.sch_df = None  # シフト表を表すデータフレーム

    def set_data(self, staff_df, calendar_df, weight_df, ng_df):
        # リストの設定
        ...
        self.weight = weight_df.set_index("スタッフID")["重要度"].to_dict()
        self.ng = ng_df.set_index(["スタッフID", "日付"])["休暇希望"].to_dict()
    ...
    def build_model(self):
        ...
        for d in self.D:
            self.model += (
                pulp.lpSum(self.x[s, d] * self.ng[s, d] for s in self.S)
                >= self.D2required_staff[d]
            )
        ...
        self.model += pulp.lpSum(
            [self.weight[s] * (self.y_under[s] + self.y_over[s]) for s in self.S]
        )
